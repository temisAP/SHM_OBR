    ...
    for j, state in enumerate(['P','S']):
        ...
        for i, c_axis in enumerate(['Real','Imaginary']):
            ...
            for key, slice in slices.items():
                ...
                # Zero crossings
                bpm_wave, _ = librosa.beat.beat_track(y = wave, sr = new_sr,
                                                tightness=100, trim=True, units='time')

                # Assignation to later plot

                delta_T_list.append(delta_T)
                delta_EPS_list.append(delta_EPS)
                bpm_list.append(bpm_wave)


            # Display real and imaginary values
            im = ax[i,j].scatter(delta_T_list,delta_EPS_list,c=bpm_list,marker = 'x',cmap='coolwarm')
